...
def available_mentors(expertise = None, language = None, available = True):
    ...
    try:
        ...
        for mentor in mentors:
            ...
            mentor_id = mentor.id
            mentor_lst.append({'id': mentor_id, 'name': mentor_info['name'], 'email': mentor_info['email'], 'expertise': mentor_info.get('expertise', 'unknown'), 'language': mentor_info.get('language', 'unknown')})
            
            click.echo(f" - Name: {mentor_info['name']}, Email: {mentor_info['email']},Expertise: {mentor_info.get('expertise','unknown')}, Language: {mentor_info.get('language', 'unknown')}")
...
def available_peers(expertise=None, language = None, available=True):
    ...
    try:
        ...
        for peer in peers:
            ...
            peers_lst.append({"id": peer.id, 'name': peer_info['name'],'email': peer_info['email'], 'expertise': peer_info.get('expertise', 'unknown'), 'language': peer_info.get('language', 'unknown')})
            click.echo(f" - Name: {peer_info['name']}, Email: {peer_info['email']}, Expertise: {peer_info.get('expertise', 'unknown')}, Language: {peer_info.get('language', 'unknown')}")
